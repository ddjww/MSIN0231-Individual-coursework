...
- Length: 420–450 words (MUST be < 480).To reach this length, expand Paragraph 3 and 4 with deep analysis.
- Structure: EXACTLY 4 long, analytical paragraphs. If you write a 5th paragraph, the task fails. No headings. No bullet points.
...
if st.button("Generate"):
    if not api_key:
        ...
        st.session_state.docs = None  # 确保这行存在
        st.session_state.wiki_context = "" # 确保这行存在
        if "report_text" in st.session_state:
            st.session_state.report_text = None
            del st.session_state["report_text"] # 彻底删除旧报告
        st.rerun()
